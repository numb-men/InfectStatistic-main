...
class InfectStatistic {
    ...
    static class LogDao {
        ...
        public static List<LogUtil.InfectResult> initLog(String fileName, String date)  {
            ...
            //如果为文件目录
                ...
                for(String file_path :paths){
                    ...
                    String result = file_path.substring(index);
                    //截取\之后字符串
                    ...
                    int res = str2.compareTo(date);

                    if(date.equals("default")) res = 0;
         ...
         * @return void
         */
        public static void outLog(String fileName, String out_path, String commandline, List<String> province, List<String> type, String date) throws IOException {

            //获取所有行
            List<LogUtil.InfectResult> list = sortResultByProvince(fileName, date);
...
//                                        province_not_found = pr.get(j) + ip + sp + cure + dead + "\n";
//                                    }
//
//                                }
//                                else{
...
//                                    }
//                                    for(int j = 0 ; j<pr.size(); j++) {
//                                        province_not_found = pr.get(j) + " 感染患者0人" + " 疑似患者0人" + " 治愈0人" + " 死亡0人" + "\n";
//                                    }
//                                }
//                            }
                        }
    ...
    static class CommandLineApplication {
        ...
        public void Application(List<String> list, String commandline) throws IOException {
            ...
            if(commandLine.getCommand().isList()){
                if (commandLine.getArguments().isLog()){
                    if(commandLine.getArguments().isDate()){
                        if(commandLine.getArguments().getDate_content()!=null) {
                            in_path = commandLine.getArguments().getLog_content();
                            date = commandLine.getArguments().getDate_content();
            ...
            LogDao.sortResultByProvince(in_path, date);

            LogDao.outLog(in_path, out_path, commandline, province, type, date);
    ...
    public static void main(String[] args) throws IOException {
        ...
        //处理-date后不设置则默认为所提供日志最新的一天。你需要确保你处理了指定日期之前的所有log文件
