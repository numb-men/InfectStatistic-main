...
class TestInfectStatistic(unittest.TestCase):
    ...
    def _compare_file(self, file_right, file_test):
        ...
        for i in range(min(len(file_right_line), len(file_test_line))):
            self.assertEqual(file_right_line[i].strip(), file_test_line[i].strip(), '第 {} 行 内容不一致'.format(i + 1))

    def _test_case(self, logs_path, right_out_name, out_name, date=None, allows_types=None, province=None):
        ...
        with open(get_result_name(right_out_name), 'r', encoding='utf-8') as file_right:
            with open(get_result_name(out_name), 'r', encoding='utf-8') as file_test:
    ...
    def test_types_1(self):
        self._test_case(_test_log, 'result4.txt', 'test_out.txt', None, ['sp', 'dead', 'cure'], None)
    ...
    def test_all_args_1(self):
        self._test_case(_test_log, 'result9.txt', 'test_out.txt', '2020-01-22', ['sp', 'ip'], ['福建'])
